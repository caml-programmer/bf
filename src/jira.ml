...
(* TO-DO:
 *
 * Нужно сделать формирование chanelog-ов по компонентам и пакетам в
 * модуле Package? после чего после сборки пакета производить вычисление
 * changelog-ов с последующий фиксацией багов в Jira.
 *)
