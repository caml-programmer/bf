...
let open_logfile () =
  let logfile =
    (with_exn "log-dir not found" (get_env "log-dir")) ^ "/" ^
    (with_alt "bf" (get_env "component"))
  in
...
let log_command prog args =
  ...
  with_logger
	...
	let cmd = Shell.cmd ~cmdname:program progam args in
	...
	log_message ~logger (sprintf "success: %s" cmd_s)
      with Shell.Subproces_error errors ->
	List.iter
	(fun (cmd,ps) ->
	  (match ps with
	    | Unix.WEXITED rc ->
